...
namespace RegexCrafter.Helpers
{
    public class StashTab(RegexCrafter core)
    {
        ...
        {
            if (!IsVisible || TabType != InventoryType.CurrencyStash)
            {
                return false;
            }
            if (!TryGetCurrencySwitchButton(typeButton, out var button))
            {
                return false;
            }

            if (!await button.MoveAndClick())
            {
                return false;
            }
        ...
        {
            if (TabType != InventoryType.CurrencyStash)
            {
                return CurrencyTabType.None;
            }
            if (Inventory.Children[1].IsVisible)
            {
                return CurrencyTabType.General;
            }
