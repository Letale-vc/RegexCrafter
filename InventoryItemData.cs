...
public partial class InventoryItemData
{
    ...
    public InventoryItemData(Element el)
    {
        try
        {
            ...
            {
                if (req.intelligence > 0) Requirements.Add($"Int: {req.intelligence}");
                if (req.strength > 0) Requirements.Add($"Str: {req.strength}");
                if (req.dexterity > 0) Requirements.Add($"Dex: {req.dexterity}");
            }
    ...
    public List<string> GetCustomTextLines()
    {
        ...
        if (!string.IsNullOrEmpty(UniqueName)) textList.Add(UniqueName);
        if (!string.IsNullOrEmpty(Sockets)) textList.Add($"Sockets: {Sockets}");
        if (Quality > 0) textList.Add($"Quality: {Quality}%");
        if (IsCorrupted) textList.Add("Corrupted");
        ...
        if (IsRedeemer) textList.Add("Redeemer");
        if (IsHunter) textList.Add("Hunter");
